...
size_t cantidad_delimitador(const char* str, char sep){
    size_t apariciones = 0;
    for(size_t i = 0; i < strlen(str); i++) if(str[i] == sep) apariciones++;
    return apariciones;
}

size_t aparicion_delimitador(const char* str, char sep){
    size_t i = 0;
...
char *substr(const char *str, size_t n){
    ...
    char* nueva_cadena = (n > largo_cadena) ? malloc(sizeof(char) * largo_cadena + 1) : malloc(sizeof(char*) * (n + 1));
    if(!nueva_cadena) return NULL;
    if(n > largo_cadena){
        strcpy(nueva_cadena, str);
        nueva_cadena[largo_cadena] = '\0';
    }else{
...
char **split(const char *str, char sep){
    if(!str) return NULL;
    size_t i, delimitadores = cantidad_delimitador(str, sep) + 1;
    ...
    for(i = 0; i < delimitadores; i++){
        size_t caracteres = aparicion_delimitador(str_aux, sep);
